...
export default function GoogleCalendarButton() {
  ...
  // Pick up access token after OAuth redirect
  useEffect(() => {
    ...
    if (token) {
      ...
      window.history.replaceState({}, '', window.location.pathname)
    } else if (sessionStorage.getItem('google_access_token')) {
      setIsAuthed(true)
    }
  }, [])
  ...
  const handleGoogleAuth = () => {
    ...
    if (!clientId || clientId === 'YOUR_GOOGLE_CLIENT_ID') {
      showToast(
        'To use Google Calendar, add VITE_GOOGLE_CLIENT_ID to your .env.local file',
        'info'
      )
      setIsAuthed(true)
      return
    ...
    // Redirect in same window so callback can redirect back
  ...
  return (
    ...
    >
      ...
      {isAuthed ? '✅ Google Calendar Ready' : 'Connect Google Calendar'}
    </button>
  )
}
